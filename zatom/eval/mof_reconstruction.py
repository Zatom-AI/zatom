...
class MOFReconstructionEvaluator:
    ...
    def get_wandb_table(self, current_epoch: int = 0, save_dir: str = "") -> wandb.Table:
        ...
        for idx in range(len(self.pred_mof_list)):
            ...
            true_lengths = " ".join(
                [f"{ln:.2f}" for ln in self.gt_arrays_list[idx]["lengths"].squeeze()]
            )
            ...
            pred_lengths = " ".join(
                [f"{ln:.2f}" for ln in self.pred_arrays_list[idx]["lengths"].squeeze()]
            )
...
def array_dict_to_structure(
    ...
    try:
        ...
        log.warning(f"Failed to convert array to Structure due to: {e}")
