...
class WeightAveraging(Callback):
    r"""A callback that updates an averaged model for Stochastic Weight Averaging (SWA) or
    Exponential Moving Average (EMA) after each training step.

    Arguments given to the constructor will be passed to the `AveragedModel` constructor. If no ``device`` is
    specified, the device of the original model will be used. Contrary to `AveragedModel`, ``use_buffers`` is set
    ...
    `AveragedModel` class for details. If no averaging function is provided, the default is to compute the
    ...
    See also the documentation on the `weight averaging callbacks <advanced/training_tricks:Weight Averaging>`
    provided by Lightning.

    Note:
        To ensure that the `AveragedModel` will contain all layers, ``setup()`` will call
        `~lightning.pytorch.core.hooks.ModelHooks.configure_model` before instantiating the
        `AveragedModel`. However, that hook is not called in a strategy aware context, sharded models do not work
    ...
    Args:
        device: By default, the `AveragedModel` will be stored on the same device as the original model. If the
            ``device`` argument is provided, the `AveragedModel` will be stored on this device instead. If you
            run out of GPU memory, you might want to use ``"cpu"``.
        use_buffers: If ``False``, the buffers of the model will not be averaged.
        kwargs: Additional keyword arguments to be passed to the `AveragedModel` constructor, such as ``avg_fn``
            or ``multi_avg_fn``.
    """
    ...
    def should_update(
        ...
        """Called after every optimizer step and after every training epoch to check whether the
        average model should be updated.
    ...
    def on_load_checkpoint(
        ...
        if self._average_model is None:
            ...
            average_model_state = {
                "module." + name: value for name, value in checkpoint["state_dict"].items()
            }
    ...
    def _swap_models(self, pl_module: "pl.LightningModule") -> None:
        """Swaps the parameter values of the current model and the `AveragedModel`.
    ...
    def _copy_average_to_current(self, pl_module: "pl.LightningModule") -> None:
        """Copies the parameter values from the `AveragedModel` to the current model.
