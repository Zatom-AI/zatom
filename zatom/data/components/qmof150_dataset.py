...
from zatom.data.components.preprocessing_utils import build_crystal, build_crystal_graph
...
class QMOF150(InMemoryDataset):
    ...
    def process(self) -> None:
        ...
        for data_dict in cached_data:
            ...
            # Only retain metal-organic frameworks (MOFs) with under 150 atoms in unit cell
            if num_atoms <= 150:
...
@typecheck
def process_one(data_dir: str, filename: str) -> Dict[str, Any] | None:
    """Process a single crystal structure file.

    Args:
        data_dir: The directory containing the crystal structure files.
        filename: The name of the crystal structure file to process.
    ...
    crystal = build_crystal(crystal_str, niggli=True, primitive=False)
    result_dict = {}
    result_dict["spacegroup"] = 1  # NOTE: Currently do not compute the spacegroup...
    graph_arrays = build_crystal_graph(crystal, "none")
    ...
    for k, v in desc.items():
        if isinstance(v, bool):
            if k == "has_3d_connected_graph":
                # NOTE: <has_3d_connected_graph> is introduced in a newer version of MOFCHecker.
