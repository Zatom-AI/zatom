...
#SBATCH --job-name=eval-sweep-tft-160M-geom                   # job name
#SBATCH --output=scripts/perlmutter/regular/logs/eval_sweep_tft_160M_geom%j.out  # output log file
#SBATCH --error=scripts/perlmutter/regular/logs/eval_sweep_tft_160M_geom%j.err   # error log file
#SBATCH --array=0-7                                           # create an array of jobs for the sweep (0-11 or 12 total for finetuning and 0-7 or 8 total for generative evaluation)
...
# Define run details
DEFAULT_SWEEP_ID="ngyat6bu"                   # NOTE: Generate a unique ID for each run by running `wandb sweep configs/sweep/{train,eval}_sweep_{joint,}.yaml`
