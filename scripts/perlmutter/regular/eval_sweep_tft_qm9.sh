...
#SBATCH --job-name=eval-sweep-tft-qm9                         # job name
#SBATCH --output=scripts/perlmutter/regular/logs/eval_sweep_tft_qm9%j.out  # output log file
#SBATCH --error=scripts/perlmutter/regular/logs/eval_sweep_tft_qm9%j.err   # error log file
...
# Define run details
DEFAULT_SWEEP_ID="4wnyi2aq"                   # NOTE: Generate a unique ID for each run by running `wandb sweep configs/sweep/{train,eval}_sweep_{joint,}.yaml`
