...
#SBATCH --job-name=eval-sweep-tft-mp20-only                   # job name
#SBATCH --output=scripts/perlmutter/regular/logs/eval_sweep_tft_mp20-only%j.out  # output log file
#SBATCH --error=scripts/perlmutter/regular/logs/eval_sweep_tft_mp20-only%j.err   # error log file
...
# Define run details
DEFAULT_SWEEP_ID="l3y9awmv"                   # NOTE: Generate a unique ID for each run by running `wandb sweep configs/sweep/{train,eval}_sweep_{joint,}.yaml`
