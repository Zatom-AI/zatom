...
#SBATCH --job-name=eval-sweep-tft-300M-geom                   # job name
#SBATCH --output=scripts/perlmutter/regular/logs/eval_sweep_tft_300M_geom%j.out  # output log file
#SBATCH --error=scripts/perlmutter/regular/logs/eval_sweep_tft_300M_geom%j.err   # error log file
...
# Define run details
DEFAULT_SWEEP_ID="peocb1os"                   # NOTE: Generate a unique ID for each run by running `wandb sweep configs/sweep/{train,eval}_sweep_{joint,}.yaml`
