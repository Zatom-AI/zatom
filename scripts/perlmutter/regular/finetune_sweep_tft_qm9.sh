...
#SBATCH --job-name=finetune-sweep-tft-qm9                     # job name
#SBATCH --output=scripts/perlmutter/regular/logs/finetune_sweep_tft_qm9%j.out  # output log file
#SBATCH --error=scripts/perlmutter/regular/logs/finetune_sweep_tft_qm9%j.err   # error log file
#SBATCH --array=0-23                                          # create an array of jobs for the sweep (0-23 or 24 total for finetuning and 0-23 or 24 total for generative evaluation)
...
# Define run details
DEFAULT_SWEEP_ID="pczrzajr"                   # NOTE: Generate a unique ID for each run by running `wandb sweep configs/sweep/{train,eval}_sweep_{joint,}.yaml`
