...
#SBATCH --job-name=mft-500M-x1                                # job name
...
DEFAULT_RUN_ID="wnt3rh88"                 # NOTE: Generate a unique ID for each run using `python scripts/generate_id.py`
DEFAULT_RUN_DATE="2025-10-20_12-00-00"    # NOTE: Set this to the initial date and time of the run for unique identification (e.g., ${now:%Y-%m-%d}_${now:%H-%M-%S})
DEFAULT_ARCHITECTURE="mft_500M"           # NOTE: Set the model architecture to be used, must be one of (`met_80M`, `met_180M`, `met_500M`, `mft_80M`, `mft_180M`, `mft_500M`)
...
RUN_NAME="train_arch-${ARCHITECTURE}_${DATASET}_x1_pred"      # Name of the model type and dataset configuration
...
bash -c "
    ...
    model.architecture.continuous_x_1_prediction=true \
    name=$RUN_NAME \
    task_name=$TASK_NAME \
