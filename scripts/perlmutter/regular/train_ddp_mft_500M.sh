...
#SBATCH --job-name=mft-500M                                   # job name
...
DEFAULT_RUN_ID="gdhqaoop"                 # NOTE: Generate a unique ID for each run using `python scripts/generate_id.py`
...
DEFAULT_ARCHITECTURE="mft_500M"           # NOTE: Set the model architecture to be used, must be one of (`{mft,met,mfp}_80M`, `{mft,met,mfp}_180M`, `{mft,met,mfp}_500M`)
