...
#SBATCH --job-name=mft-l                                      # job name
...
DEFAULT_RUN_ID="bg5gnr8k"                 # NOTE: Generate a unique ID for each run using `python scripts/generate_id.py`
DEFAULT_RUN_DATE="2025-09-29_14-30-00"    # NOTE: Set this to the initial date and time of the run for unique identification (e.g., ${now:%Y-%m-%d}_${now:%H-%M-%S})

DATASET=${1:-$DEFAULT_DATASET}            # First argument or default dataset if not provided
RUN_NAME="MFT-L__${DATASET}"              # Name of the model type and dataset configuration
...
bash -c "
    ...
    callbacks=$CALLBACKS \
    data=$DATASET \
    data.datamodule.batch_size.train=720 \
    data.datamodule.batch_size.val=720 \
    data.datamodule.batch_size.test=720 \
    date=$RUN_DATE \
    ecoder=mft \
