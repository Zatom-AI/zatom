...
#SBATCH --job-name=finetune-tft-5M-joint                      # job name
...
DEFAULT_RUN_ID="05rfrq49"                 # NOTE: Generate a unique ID for each run using `python scripts/generate_id.py`
DEFAULT_RUN_DATE="2025-11-20_11-30-00"    # NOTE: Set this to the initial date and time of the run for unique identification (e.g., ${now:%Y-%m-%d}_${now:%H-%M-%S})
...
bash -c "
    ...
    model.augmentations.frac_coords=false \
    model.augmentations.pos=false \
