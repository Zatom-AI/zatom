...
DEFAULT_RUN_ID="cz2z0mcu"                 # NOTE: Generate a unique ID for each run using `python scripts/generate_id.py`
...
bash -c "
    ...
    +model.architecture.multimodal_model.qk_layernorm=true \
    name=$RUN_NAME \
    task_name=$TASK_NAME \
