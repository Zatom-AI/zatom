...
#        --ntasks-per-node=4 \
#        --time=04:00:00 \
#        --job-name=tft-70M-joint-ape-no-trfmr-in-norm
...
DEFAULT_RUN_ID="o26izbh4"                 # NOTE: Generate a unique ID for each run using `python scripts/generate_id.py`
...
RUN_NAME="${EXPERIMENT}_model-${MODEL}_arch-${ARCHITECTURE}_joint_ape_no_trfmr_in_norm"     # Name of the model type and dataset configuration
