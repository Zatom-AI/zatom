...
DEFAULT_RUN_ID="r2gj5a9s"                 # NOTE: Generate a unique ID for each run using `python scripts/generate_id.py`
...
bash -c "
    ...
    data.datamodule.batch_size.val=1024 \
    data.datamodule.batch_size.test=1024 \
