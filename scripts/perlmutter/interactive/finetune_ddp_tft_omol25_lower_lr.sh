...
#        --ntasks-per-node=4 \
#        --time=04:00:00 \
#        --job-name=finetune-tft-80M-omol25-lower-lr
...
DEFAULT_RUN_ID="tigvz4ew"                 # NOTE: Generate a unique ID for each run using `python scripts/generate_id.py`
DEFAULT_RUN_DATE="2026-01-05_06-00-00"    # NOTE: Set this to the initial date and time of the run for unique identification (e.g., ${now:%Y-%m-%d}_${now:%H-%M-%S})
...
RUN_NAME="${EXPERIMENT}_model-${MODEL}_arch-${ARCHITECTURE}_omol25_lower_lr"     # Name of the model type and dataset configuration
