...
DEFAULT_RUN_ID="dus63051"                 # NOTE: Generate a unique ID for each run using `python scripts/generate_id.py`
...
bash -c "
    ...
    data=$DATASET \
    data.datamodule.batch_size.train=96 \
    data.datamodule.batch_size.val=96 \
    data.datamodule.batch_size.test=96 \
