...
#        --ntasks-per-node=2 \
#        --time=04:00:00 \
#        --job-name=finetune-tft-80M-matbench-qm9-only
...
DEFAULT_RUN_ID="60o406xb"                 # NOTE: Generate a unique ID for each run using `python scripts/generate_id.py`
DEFAULT_RUN_DATE="2025-12-27_09-00-00"    # NOTE: Set this to the initial date and time of the run for unique identification (e.g., ${now:%Y-%m-%d}_${now:%H-%M-%S})
...
RUN_NAME="${EXPERIMENT}_model-${MODEL}_arch-${ARCHITECTURE}_qm9_only_matbench"       # Name of the model type and dataset configuration

PRETRAINED_CKPT_PATH="logs/train_fm/runs/train_model-zatom_arch-tft_80M_QM9_2025-12-21_09-00-00/checkpoints/model-epoch@399-step@9600-val_qm9_valid_rate@0.9677-val_mp20_valid_rate@0.0000.ckpt"  # Path at which to find (initial) pretrained model checkpoint
