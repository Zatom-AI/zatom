...
DEFAULT_RUN_ID="bd07x382"                 # NOTE: Generate a unique ID for each run using `python scripts/generate_id.py`
DEFAULT_RUN_DATE="2025-12-15_19-00-00"    # NOTE: Set this to the initial date and time of the run for unique identification (e.g., ${now:%Y-%m-%d}_${now:%H-%M-%S})
...
bash -c "
    ...
    ckpt_path=$CKPT_PATH \
    data=$DATASET \
    data.datamodule.batch_size.base_world_size=null \
    date=$RUN_DATE \
