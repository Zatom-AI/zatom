...
#        --ntasks-per-node=2 \
#        --time=04:00:00 \
#        --job-name=finetune-tft-80M-omol25-larger-bb
...
DEFAULT_RUN_ID="8mbh6mjs"                 # NOTE: Generate a unique ID for each run using `python scripts/generate_id.py`
DEFAULT_RUN_DATE="2026-01-05_20-00-00"    # NOTE: Set this to the initial date and time of the run for unique identification (e.g., ${now:%Y-%m-%d}_${now:%H-%M-%S})
...
RUN_NAME="${EXPERIMENT}_model-${MODEL}_arch-${ARCHITECTURE}_omol25_larger_bb"    # Name of the model type and dataset configuration
