...
#        --ntasks-per-node=4 \
#        --time=04:00:00 \
#        --job-name=tft-70M-joint-ape-no-trfmr-in-norm--short-context
...
DEFAULT_RUN_ID="d10bftxk"                 # NOTE: Generate a unique ID for each run using `python scripts/generate_id.py`
DEFAULT_RUN_DATE="2025-12-14_13-00-00"    # NOTE: Set this to the initial date and time of the run for unique identification (e.g., ${now:%Y-%m-%d}_${now:%H-%M-%S})
...
RUN_NAME="${EXPERIMENT}_model-${MODEL}_arch-${ARCHITECTURE}_joint_ape_no_trfmr_in_norm__short_context"     # Name of the model type and dataset configuration
...
bash -c "
    ...
    globals.max_num_atoms=450 \
    model=$MODEL \
    model/architecture=$ARCHITECTURE \
    model.architecture.multimodal_model.add_sinusoid_posenc=true \
    model.architecture.multimodal_model.normalize_transformer_input=false \
    name=$RUN_NAME \
    task_name=$TASK_NAME \
